...
func AccountCloseTxn(addr basics.Address, stxn transactions.SignedTxnWithAD) bool {
	if protocol.TxType(stxn.Txn.Type) != protocol.PaymentTx {
		return false
	}
	return !stxn.Txn.CloseRemainderTo.IsZero() && basics.Address(stxn.Txn.Sender) == addr
}
...
func AssetCreateTxn(stxn transactions.SignedTxnWithAD) bool {
	if protocol.TxType(stxn.Txn.Type) != protocol.AssetConfigTx {
		return false
	}
...
func AssetOptInTxn(stxn transactions.SignedTxnWithAD) bool {
	if protocol.TxType(stxn.Txn.Type) != protocol.AssetTransferTx {
		return false
	}
...
func AppCreateTxn(stxn transactions.SignedTxnWithAD) bool {
	if protocol.TxType(stxn.Txn.Type) != protocol.ApplicationCallTx {
		return false
	}
...
func AppDestroyTxn(stxn transactions.SignedTxnWithAD) bool {
	...
	return transactions.OnCompletion(stxn.Txn.OnCompletion) == transactions.DeleteApplicationOC
}
...
func AppOptInTxn(stxn transactions.SignedTxnWithAD) bool {
	...
	return transactions.OnCompletion(stxn.Txn.OnCompletion) == transactions.OptInOC
}
...
func AppOptOutTxn(stxn transactions.SignedTxnWithAD) bool {
	...
	return transactions.OnCompletion(stxn.Txn.OnCompletion) == transactions.CloseOutOC || transactions.OnCompletion(stxn.Txn.OnCompletion) == transactions.ClearStateOC
}

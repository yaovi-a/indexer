...
import (
	...
	// Load the postgres pgxpool.Pool implementation.
...
// of sql.Tx.Commit().
func (db *IndexerDb) txWithRetry(ctx context.Context, opts pgx.TxOptions, f func(context.Context, pgx.Tx) error) error {
	count := 0
	for {
...
func (db *IndexerDb) getMetastate(tx pgx.Tx, key string) (string, error) {
	query := `SELECT v FROM metastate WHERE k = $1`

	var row pgx.Row
	if tx == nil {
		row = db.db.QueryRow(context.Background(), query, key)
	} else {
		row = tx.QueryRow(context.Background(), query, key)
	}
...
// Returns idb.ErrorNotInitialized if uninitialized.
// If `tx` is nil, use a normal query.
func (db *IndexerDb) getImportState(tx pgx.Tx) (importState, error) {
...
// If `tx` is nil, use a normal query.
func (db *IndexerDb) getNextRoundToAccount(tx pgx.Tx) (uint64, error) {
...
// Returns ErrorNotInitialized if genesis is not loaded.
// If `tx` is nil, use a normal query.
func (db *IndexerDb) getMaxRoundAccounted(tx pgx.Tx) (uint64, error) {
...
func (db *IndexerDb) yieldTxns(ctx context.Context, tx pgx.Tx, tf idb.TransactionFilter, out chan<- idb.TxnRow) {
	...
	rows, err := tx.Query(context.Background(), query, whereArgs...)
	if err != nil {
		err = fmt.Errorf("txn query %#v err %v", query, err)
		out <- idb.TxnRow{Error: err}
		return

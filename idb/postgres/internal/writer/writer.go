...
func (w *Writer) addTransactions(block *bookkeeping.Block, modifiedTxns []transactions.SignedTxnInBlock) error {
	f := func(i int) ([]interface{}, error) {
		...
		stxnad.SignedTxn, stxnad.ApplyData, err =
			block.BlockHeader.DecodeSignedTxn(block.Payset[i])
		if err != nil {
			return nil, fmt.Errorf("addTransactions() decode signed txn err: %w", err)
		}

		txn := &stxnad.Txn
		typeenum, ok := idb.GetTypeEnum(txn.Type)
		if !ok {
			return nil, fmt.Errorf("addTransactions() get type enum")
		}
		...
		res := []interface{}{uint64(block.Round()), i, int(typeenum), assetid, id,
	...
	_, err := w.tx.CopyFrom(
		context.Background(), pgx.Identifier{"txn"},
		[]string{"round", "intra", "typeenum", "asset", "txid", "txnbytes", "txn", "extra"},
		pgx.CopyFromSlice(len(block.Payset), f))
	if err != nil {

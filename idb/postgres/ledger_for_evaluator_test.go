...
func TestLedgerForEvaluatorBlockHdr(t *testing.T) {
	...
	_, err = db.db.Exec(context.Background(), query, encoding.EncodeBlockHeader(header))
...
func TestLedgerForEvaluatorAccountAssetTable(t *testing.T) {
	...
	query =
		"INSERT INTO account_asset (addr, assetid, amount, frozen, deleted, created_at) " +
			"VALUES ($1, $2, $3, $4, $5, 0)"
	_, err = db.db.Exec(context.Background(), query, test.AccountA[:], 1, 2, false, false)
	require.NoError(t, err)
	_, err = db.db.Exec(context.Background(), query, test.AccountA[:], 3, 4, true, false)
	require.NoError(t, err)
	_, err = db.db.Exec(context.Background(), query, test.AccountA[:], 5, 6, true, true) // deleted
	require.NoError(t, err)
	_, err = db.db.Exec(context.Background(), query, test.AccountB[:], 5, 6, true, false) // wrong account
	require.NoError(t, err)
...
func TestLedgerForEvaluatorAssetTable(t *testing.T) {
	...
	_, err = db.db.Exec(context.Background(), query, 3, test.AccountA[:], "{}", true) // deleted
	require.NoError(t, err)

	_, err = db.db.Exec(context.Background(), query, 4, test.AccountD[:], "{}", false) // wrong account
	require.NoError(t, err)
